...
class DeepRacerEnv(gym.Env):
    ...
    def reward_function(self, on_track, x, y, distance_from_center, car_orientation, progress, steps,
        ...
        # penalize the car is not driving safely
        rewards += 5 * (np.tanh(-distance_from_center**2) + 1 + 1e-3)

        # penalize reward if the car steering too much
        rewards *= (np.tanh(-(steering - self.steering_angle)**2)+1+1e-3)

        next_point = waypoints[closest_waypoints[0]]
        prev_point = waypoints[closest_waypoints[1]]

        track_direction = math.atan2(
            next_point[1] - prev_point[1], next_point[0] - prev_point[0])
        track_direction = math.degrees(track_direction)
        direction_diff = abs(track_direction - math.degrees(car_orientation))
        direction_diff = min(direction_diff, 360 - direction_diff)
        direction_reward = direction_diff * 1e-3
        rewards += direction_reward

        # add on the remaining reward
        rewards += (progress - self.prev_progress) * 0.5 * 1e-3
        self.prev_progress = progress

        # add on the remaining reward
        rewards += (on_track * 1.0) * 1e-3
